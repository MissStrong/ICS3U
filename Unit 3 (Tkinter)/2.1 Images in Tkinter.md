...
We can place images on a canvas using `create_image()`. The parameters are `x` and `y` (the x- and y-coordinates of the center of the image), followed by the keyword argument `image`, which takes a `PhotoImage`. `PhotoImage()` takes keyword arguments including `file` for the filepath of the image. 
