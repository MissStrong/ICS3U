...
Most functions in Tkinter that take a colour as a parameter need the colour to be expressed as a hex value. For convenience, we can make constants for the colours we use in our program.
...
The default colour of a window in Tkinter is grey. We can change the colour of the screen using the `.configure()` method on the window and passing the background colour as a keyword argument like this:
