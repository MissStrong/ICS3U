...
Recursion can be used to calculate the greatest common factor (GCF) between two integers using the identities `gcf(a, b) == gcf(b, a % b)` and `gcf(a, 0) == a`, where `%` is the modulo operator. This process is called the Euclidean algorithm.
