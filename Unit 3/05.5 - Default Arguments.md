...
Recall that `range()` can take one, two, or three arguments represnting `start`, `stop`, and `step`. If we don't specify the `step` then its default value is `1` and if we don't specify `start` then its default value is `0`.
...
Suppose we want to create a similar function that has **default arguments**. We can indicate default arguments in the signature of the function using the `=` operator.
